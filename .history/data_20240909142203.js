        ...
        itemElement.innerHTML = `
            ...
            <div class="buy-content buy-content${item.index}">
              ...
              <div class="item-buy-action">
                <svg class="delete-${item.index}" onclick="deleteItem(${item.index})" width="14" height="14" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <div class="confirm-delete">
              <div class="confirm-delete-left">
                <p>Add to wishlist before remove?</p>
                <div class="btn-delete-order">
                  <button class="btn-yes" onclick="removeItem(${item.index})" aria-label="btn-yes1">YES</button>
                  <button class="btn-no" aria-label="btn-yes2">NO</button>
                </div>
              </div>
              <div class="confirm-delete-right">
...
function deleteItem(itemId) {
    // Tìm index của item theo ID
    const indexToDelete = cartItems.findIndex(item => item.id === itemId);
  
    if (indexToDelete !== -1) {
      cartItems.splice(indexToDelete, 1); // Xóa item từ mảng cartItems
      renderCart(); // Cập nhật lại giao diện giỏ hàng
    }
  }

    function deleteItem(index) {
        // Lấy các phần tử DOM dựa trên chỉ số index
        const removeBtn1 = document.querySelector(`.delete-item${index}`); // Nút xóa của sản phẩm
        const buycontent1 = document.querySelector(`.buy-content${index}`); // Nội dung sản phẩm
        const confirmdelete1 = document.querySelector(`.confirm-delete${index}`); // Xác nhận xóa
        const confirmdelete1right = document.querySelector(`.confirm-delete${index}-right`); // Phần tử trong phần xác nhận
        const btnyes1 = document.querySelector(`.btn-yes${index}`); // Nút xác nhận "Có"
        const btnno1 = document.querySelector(`.btn-no${index}`); // Nút xác nhận "Không"
        const itembuy1 = document.getElementById(`itembuy-delete${index}`); // Phần tử giỏ hàng sản phẩm
        ...
        btnyes1.addEventListener('click', function() {
            ...
            // Có thể gọi thêm logic xóa sản phẩm khỏi giỏ hàng ở đây
        });
