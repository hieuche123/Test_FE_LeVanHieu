...
function showSlides(n) {
  if (!isDesktop()) return; // Chỉ thực thi trên giao diện máy tính
...
// Hàm kiểm tra nếu giao diện đang ở chế độ desktop (>= 768px)
function isDesktop() {
  return window.matchMedia("(min-width: 768px)").matches;
}

// Lắng nghe sự thay đổi kích thước cửa sổ để cập nhật
window.addEventListener('resize', () => {
  if (!isDesktop()) {
    // Ẩn tất cả slide và reset lại trạng thái nếu chuyển sang chế độ mobile
    let slides = document.getElementsByClassName("slide");
    for (let i = 0; i < slides.length; i++) {
    }
  } else {
    // Hiển thị slide hiện tại khi quay lại chế độ desktop
    showSlides(slideIndex);
  }
});
